

/*
    json:
        originalLanguage = 원본 언어
        number = 회차 번호
        type = 클라우드 파일 유형
*/
function getHtmlPopupContents_part_user_translation(data) {
    let html = "";

    let originalLanguage = data["originalLanguage"];
    let selectLanguage = getUserNovelTranslationCreateLanguageSelectList(originalLanguage)[0]["value"];

    html = `
        <div class = "edit_profile_picture">
            <div class = "popup_contents_top">
                <div class = "popup_contents_top_left md-ripples" onclick = "backPopupContents();" onmouseenter = "hoverInformation(this, getLanguage('popup_contents:back'));">
                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="50" height="50" viewBox="0 0 50 50"><defs><clipPath id="b"><rect width="50" height="50"></rect></clipPath></defs><g id="a" clip-path="url(#b)"><g transform="translate(-12.086 -0.24)"><rect width="3" height="35" rx="1.5" transform="translate(36.399 0.479) rotate(45)"></rect><rect width="3" height="35" rx="1.5" transform="translate(38.521 47.879) rotate(135)"></rect></g><rect width="49" height="3" rx="1.5" transform="translate(1 23.5)"></rect></g></svg>
                </div>
                <div class = "popup_contents_top_right">
                    ` + getLanguage('viewer_settings_user_translation:title') + `
                </div>
            </div>
            
            <div class = "popup_part_user_translation">
                <div class = "create_work_step2_option">
                    <div class = "create_work_step2_option_title">
                        ` + getLanguage("popup_part_user_translation:language") + `
                    </div>
                    <div class = "create_work_step2_option_box md-ripples" value = "` + selectLanguage + `" onclick = "selectList(this, getUserNovelTranslationCreateLanguageSelectList('` + originalLanguage + `'));">
                        <div class = "create_work_step2_option_box_title value_title">
                            ` + getLanguage('language:' + selectLanguage) + `
                        </div>
                        <div class = "create_work_step2_option_box_icon">
                            <!-- Generated by IcoMoon.io -->
                            <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M6.984 9.984h10.031l-5.016 5.016z"></path></svg>
                        </div>
                    </div>
                </div>
                
                <div style = "margin-top: 20px;">
                    <div class = "create_work_step2_option_title">
                        ` + getLanguage("workspace_details_title:part_thumbnail") + `
                        <div class = "create_work_input_top_icon" onmouseenter = "hoverHelp(this, getLanguage('popup_workspace_part_localization_thumbnail_upload_hover_help'));">
                            <!-- Generated by IcoMoon.io -->
                            <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M12 6q1.641 0 2.813 1.172t1.172 2.813q0 1.266-1.5 2.602t-1.5 2.414h-1.969q0-1.078 0.469-1.852t1.031-1.125 1.031-0.867 0.469-1.172q0-0.797-0.609-1.383t-1.406-0.586-1.406 0.586-0.609 1.383h-1.969q0-1.641 1.172-2.813t2.813-1.172zM12 20.016q3.281 0 5.648-2.367t2.367-5.648-2.367-5.648-5.648-2.367-5.648 2.367-2.367 5.648 2.367 5.648 5.648 2.367zM12 2.016q4.125 0 7.055 2.93t2.93 7.055-2.93 7.055-7.055 2.93-7.055-2.93-2.93-7.055 2.93-7.055 7.055-2.93zM11.016 18v-2.016h1.969v2.016h-1.969z"></path></svg>
                        </div>
                    </div>
                    <div class = "workspace_work_cover" value = "0">
                        <div class = "workspace_work_cover_selected img_wrap">
                            <img src = "" onload = "imageLoad(event);" alt = "">
                        </div>
                        <div class = "workspace_work_cover_add md-ripples" onclick = "workspacePartLocalizationThumbnailUploadButton(this);">
                            <!-- Generated by IcoMoon.io -->
                            <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M16.88 9.1c1.795 0.419 3.111 2.006 3.111 3.9 0 2.206-1.786 3.995-3.991 4h-11c-0.001 0-0.003 0-0.005 0-2.761 0-5-2.239-5-5 0-2.409 1.704-4.421 3.973-4.894l0.032-0.006v-0.1c0-0.001 0-0.002 0-0.004 0-1.657 1.343-3 3-3 0.56 0 1.085 0.154 1.534 0.421l-0.014-0.008c0.899-0.889 2.136-1.438 3.5-1.438 2.75 0 4.98 2.23 4.98 4.98 0 0.017-0 0.033-0 0.050l0-0.003c0 0.38-0.040 0.74-0.12 1.1zM11 11h3l-4-4-4 4h3v3h2v-3z"></path></svg>
                            <div class = "workspace_work_cover_add_title">
                                ` + getLanguage("workspace_work_upload_cover_title") + `
                            </div>
                            <div class = "workspace_work_cover_add_description">
                                ` + getLanguage("workspace_work_upload_cover_description:localization") + `
                            </div>
                            <input type = "file" onchange = "requestWorkspacePartLocalizationThumbnailUpload(this.files[0]);" accept = "image/png, image/jpeg, image/webp, image/gif" style = "display: none;">
                        </div>
                        <div class = "workspace_work_cover_delete md-ripples" onclick = "workspacePartLocalizationThumbnailDeleteButton(this);">
                            <!-- Generated by IcoMoon.io -->
                            <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 28"><path d="M8 21.5v-11c0-0.281-0.219-0.5-0.5-0.5h-1c-0.281 0-0.5 0.219-0.5 0.5v11c0 0.281 0.219 0.5 0.5 0.5h1c0.281 0 0.5-0.219 0.5-0.5zM12 21.5v-11c0-0.281-0.219-0.5-0.5-0.5h-1c-0.281 0-0.5 0.219-0.5 0.5v11c0 0.281 0.219 0.5 0.5 0.5h1c0.281 0 0.5-0.219 0.5-0.5zM16 21.5v-11c0-0.281-0.219-0.5-0.5-0.5h-1c-0.281 0-0.5 0.219-0.5 0.5v11c0 0.281 0.219 0.5 0.5 0.5h1c0.281 0 0.5-0.219 0.5-0.5zM7.5 6h7l-0.75-1.828c-0.047-0.063-0.187-0.156-0.266-0.172h-4.953c-0.094 0.016-0.219 0.109-0.266 0.172zM22 6.5v1c0 0.281-0.219 0.5-0.5 0.5h-1.5v14.812c0 1.719-1.125 3.187-2.5 3.187h-13c-1.375 0-2.5-1.406-2.5-3.125v-14.875h-1.5c-0.281 0-0.5-0.219-0.5-0.5v-1c0-0.281 0.219-0.5 0.5-0.5h4.828l1.094-2.609c0.313-0.766 1.25-1.391 2.078-1.391h5c0.828 0 1.766 0.625 2.078 1.391l1.094 2.609h4.828c0.281 0 0.5 0.219 0.5 0.5z"></path></svg>
                            <div class = "workspace_work_cover_add_title">
                                ` + getLanguage("workspace_work_delete_cover_title") + `
                            </div>
                            <div class = "workspace_work_cover_add_description">
                                ` + getLanguage("workspace_work_delete_cover_description") + `
                            </div>
                        </div>
                    </div>
                </div>

                <div style = "margin-top: 20px;">
                    <div class = "create_work_input">
                        <div class = "create_work_input_top">
                            <div class = "create_work_input_top_title">
                                ` + getLanguage("workspace_details_title:part_title") + `
                            </div>
                        </div>
                        <input id = "upload_work_part_input_title" type = "text" placeholder = "` + getLanguage("work_settings_please_enter") + `" onkeydown = "checkButtonPopupPartUserTranslation();" onfocus = "create_work_input_focus(this);" >
                    </div>
                </div>

                <div style = "margin-top: 20px;">
                    <div class = "upload_work_part_clound_files_wrap" value = "0" onchange = "checkButtonPopupPartUserTranslation();">
                        <div class = "upload_work_part_clound_files_title">
                            <!-- Generated by IcoMoon.io -->
                            <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M2.93 17.070c-1.884-1.821-3.053-4.37-3.053-7.193 0-5.523 4.477-10 10-10 2.823 0 5.372 1.169 7.19 3.050l0.003 0.003c1.737 1.796 2.807 4.247 2.807 6.947 0 5.523-4.477 10-10 10-2.7 0-5.151-1.070-6.95-2.81l0.003 0.003zM9 5v6h2v-6h-2zM9 13v2h2v-2h-2z"></path></svg>
                            ` + getLanguage("upload_work_part_cloud_files") + `
                        </div>
                        <div class = "upload_work_part_clound_files_title_selected">
                            <!-- Generated by IcoMoon.io -->
                            <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M9.984 17.016l9-9-1.406-1.453-7.594 7.594-3.563-3.563-1.406 1.406zM12 2.016q4.125 0 7.055 2.93t2.93 7.055-2.93 7.055-7.055 2.93-7.055-2.93-2.93-7.055 2.93-7.055 7.055-2.93z"></path></svg>
                            ` + getLanguage("upload_work_part_cloud_files_selected") + `
                        </div>
                    </div>
                    <div class = "cloud_contents_info_left scroll">
                        <div class = "cloud_contents_info_left_item_wrap">
                            <div class="cloud_contents_info_left_item md-ripples" onclick = "requestUploadWorkPartCloundLoadFolder(null, '` + data["type"] + `');">
                                ` + getLanguage("my_cloud_folder") + `
                            </div>
                            <div class="cloud_contents_info_left_item_icon">
                                <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M9.707 18.707l6-6c0.391-0.391 0.391-1.024 0-1.414l-6-6c-0.391-0.391-1.024-0.391-1.414 0s-0.391 1.024 0 1.414l5.293 5.293-5.293 5.293c-0.391 0.391-0.391 1.024 0 1.414s1.024 0.391 1.414 0z"></path></svg>
                            </div>
                        </div>
                    </div>
                    <div class = "upload_work_part_clound_files_contents scroll">
                        <div class = "upload_work_part_clound_files_loading">
                            <div class="showbox"><div class="loader" style="width: 40px;"><svg class="circular" viewBox="25 25 50 50"><circle class="path" cx="50" cy="50" r="20" fill="none" stroke-width="5" stroke-miterlimit="10"></circle></svg></div></div>
                        </div>
                    </div>
                </div>

                <div class = "popup_part_user_translation_line"></div>

                <div class = "popup_part_user_translation_bottom">
                    <div class = "popup_part_user_translation_bottom_button popup_part_user_translation_bottom_button_disabled md-ripples" onclick = "requestPopupUserPartTranslation(` + data["number"] + `);">
                        <div class = "popup_part_user_translation_bottom_button_icon">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="50" height="50" viewBox="0 0 50 50"><defs><clipPath id="a"><rect width="50" height="50"></rect></clipPath></defs><g id="b" clip-path="url(#a)"><g clip-path="url(#a)"><rect width="3" height="25" rx="1.5" transform="translate(0 25)"></rect><rect width="3" height="25" rx="1.5" transform="translate(47 25)"></rect><rect width="50" height="3" rx="1.5" transform="translate(0 47)"></rect><rect width="3" height="34" rx="1.5" transform="translate(27 35.5) rotate(180)"></rect><rect width="3" height="17.526" rx="1.5" transform="translate(37.928 12.393) rotate(135)"></rect><rect width="3" height="17" rx="1.5" transform="translate(15.193 14.142) rotate(-135)"></rect></g></g></svg>
                        </div>
                        <div class = "popup_part_user_translation_bottom_button_right">
                            ` + getLanguage("popup_part_user_translation:button") + `
                        </div>
                    </div>
                </div>
            </div>

            <div style = "width: 550px;"></div>
        </div>
    `;

    setTimeout(() => {
        requestUploadWorkPartCloundFiles(data["type"]);
    }, 200);

    return html;
}

function popupUserNovelTranslationInputFocus(input) {
    let parent = input.parentElement;
    parent.classList.add("popup_part_user_translation_input_focus");
}
function popupUserNovelTranslationInputBlur(input) {
    let parent = input.parentElement;
    parent.classList.remove("popup_part_user_translation_input_focus");
}

function getUserNovelTranslationCreateLanguageSelectList(originalLanguage) {
    let items = getLanguageSelectItem();
    let newItems = new Array();
    for (let i = 0; i < items.length; i++) {
        if (originalLanguage != items[i]["value"]) {
            newItems[newItems.length] = items[i];
        }
    }
    return newItems;
}



























function checkButtonPopupPartUserTranslation() {
    function callback() {
        let popup_contents_box = document.getElementsByClassName("popup_contents_box")[0];

        //작품 제목
        let title = popup_contents_box.getElementsByClassName("create_work_input")[0];
        title = title.getElementsByTagName("input")[0].value.trim();
        //클라우드 파일 선택
        let cloudFile = popup_contents_box.getElementsByClassName("upload_work_part_clound_files_wrap")[0];
        let cloudFileValue = cloudFile.getAttribute("value");
    
        let button = popup_contents_box.getElementsByClassName("popup_part_user_translation_bottom_button")[0];
        if (title != "" && cloudFileValue != 0) {
            button.classList.remove("popup_part_user_translation_bottom_button_disabled");
        } else {
            button.classList.add("popup_part_user_translation_bottom_button_disabled");
        }
    }
    window.requestAnimationFrame(callback);
}


















function getInfoPopupUserPartTranslation() {
    let popup_contents_box = document.getElementsByClassName("popup_contents_box")[0];

    //작품 표지
    let thumbnailImage = popup_contents_box.getElementsByClassName("workspace_work_cover")[0];
    let thumbnailImageUrl = null;
    if (thumbnailImage.getAttribute("value") == "1" || thumbnailImage.getAttribute("value") == 1) {
        thumbnailImageUrl = thumbnailImage.getElementsByTagName("img")[0].src;
    }

    //회차 제목
    let title = popup_contents_box.getElementsByClassName("create_work_input")[0];
    title = title.getElementsByTagName("input")[0].value.trim();

    //언어
    language = popup_contents_box.getElementsByClassName("create_work_step2_option_box")[0].getAttribute("value");

    //클라우드 파일 번호
    let clound_files_wrap = popup_contents_box.getElementsByClassName("upload_work_part_clound_files_wrap")[0];
    let fileNumber = null;
    if (clound_files_wrap.getAttribute("value") == "1" || clound_files_wrap.getAttribute("value") == 1) {
        fileNumber = clound_files_wrap.getAttribute("fileNumber");
    }

    let data = {
        'language': language,
        'thumbnailImage': thumbnailImageUrl,
        'title': title,
        'fileNumber': fileNumber
    }

    return data;
}

















function requestPopupUserPartTranslation(partNumber) {
    let info = getInfoPopupUserPartTranslation();

    popupContentsLoading();

    const xhr = new XMLHttpRequest();
    const method = "POST";
    
    xhr.open(method, "/menu/workspace/php/part/user_translation/submission.php");
    
    xhr.addEventListener('readystatechange', function (event) {
        const { target } = event;
        if (target.readyState === XMLHttpRequest.DONE) {
            const { status } = target;
            if (status === 0 || (status >= 200 && status < 400)) {
                let xhrHtml = xhr.responseText.trim();
                
                if (xhrHtml == "created") {
                    actionMessage(getLanguage("popup_part_user_translation_message:0"));
                    history.back();
                } else if (xhrHtml == "modified") {
                    actionMessage(getLanguage("popup_part_user_translation_message:1"));
                    history.back();
                } else if (xhrHtml == "no permission") {
                    actionMessage(getLanguage("popup_part_user_translation_message:2"));
                } else if (xhrHtml == "other users only") {
                    actionMessage(getLanguage("popup_part_user_translation_message:3"));
                } else if (xhrHtml == "original language") {
                    actionMessage(getLanguage("popup_part_user_translation_message:4"));
                }
            } else {
                if (status == 504) {
                    //시간 초과
                    serverResponseErrorMessage(0);
                } else {
                    //오류 발생
                    serverResponseErrorMessage(1);
                }
            }
            popupContentsLoadingComplete();
        }
    });
    
    var formData = new FormData();
    formData.append("partNumber", partNumber);
    formData.append("language", info["language"]);
    formData.append("title", info["title"]);
    formData.append("fileNumber", info["fileNumber"]);
    if (info["thumbnailImage"] != null) {
        formData.append("thumbnailImage", info["thumbnailImage"]);
    }

    xhr.send(formData);
}